...
fn main() {
    ...
    network.add(Dense::new().set_inputs_cnt(13).set_units(5).set_activation_fn(&ActivationFn::Sigmoid).build());
    network.add(Dense::new().set_units(1).set_activation_fn(&ActivationFn::Linear).build());

    let (features_names, input, output) = FileReader::new("./datasets/housing.data", "").get_data(13);
    let error = network.fit(&input, &output, 505, 1_000);
